import serializers
import services
from database import items_db, riders_db, clock_db
...
def dispatch():
    ...
    try:
        ...
        for item in items:
            ...
            p.stdin.write(str(edd_time_algthm)+'\n')
        ...
        for i in range(num_riders):
            ...
            for delivery_ind in range(len(deliveries[i])):
                ...
                item_num_2 = (
                    deliveries[i][delivery_ind+1] + 1) if (delivery_ind+1) < len(deliveries[i]) else 0

                tasks[delivery_ind]["time_next"] = time_adj[item_num_1][item_num_2]

            if len(tasks) > 0:
                current_route, route_details, route_polyline = services.get_route(
                    tasks, -1, 0)
        ...
        for rider in riders:
            ...
            while (rider["route_index"] < len(rider["route_details"])):
                ...
                if (time_delta >= rider["route_details"][rider["route_index"]]["time_taken"]):
                    time_delta -= rider["route_details"][rider["route_index"]
                                                         ]["time_taken"]
                    ...
                    rider["route_details"][rider["route_index"]
            ...
            if rider["route_index"] == len(rider["route_details"]):
                ...
                task_completion_actual = dt.strptime(
                    "29-01-2023 23:20:03", "%d-%m-%Y %H:%M:%S")
                task_completion_simult = day_start + \
                    (task_completion_actual - clock_start)
                ...
                rider["bag_volume"] = rider["bag_volume"] + (rider["tasks"][rider["task_index"]]["bag_volume"])*(
                    1 if rider["tasks"][rider["task_index"]]["task_type"] == "Delivery" else -1)

                rider["current_route"], rider["route_details"], rider["route_polyline"] = services.get_route(
...
def add_pickup_item(item: Item):
    ...
    try:
        ...
        riders = serializers.riders_serializer(riders_db.find())
        valid_riders = [rider for rider in riders if rider["task_index"] <= (
            len(rider["tasks"]) - 2)]
        ...
        pickup_add_time_simult = day_start + (pickup_add_time_actual - clock_start)
        pickup_add_time_algthm = (pickup_add_time_simult - day_start).total_seconds()
        ...
        times_from_pickup = services.get_pickup_time_matrix(valid_riders, item)
        ...
        for rider in valid_riders:
            ...
            for task_index in range(rider["task_index"], rider["task_index"] + num_tasks):
                ...
                task_type = (0 if rider["tasks"][task_index]
                             ["task_type"] == "Delivery" else 1)
        ...
        after_task_index = int(p.stdout.readline().strip()) + \
            valid_riders[rider_ind]["task_index"]

        tasks = valid_riders[rider_ind]["tasks"]
        valid_riders[rider_ind]["tasks"] = insert_pickup(
            valid_riders[rider_ind]["rider_id"], tasks, after_task_index, item, times_from_pickup)
